...
/// first evaluated as a value, as described by [`ExpressionEvaluationStrategy`].
///
/// The truthiness of an `and`, `or`, or `not` expression is determined by its operands' truthiness,
/// so the checker visits those operands with the same evaluation strategy. For example, both
/// `enabled` and `value is not None` below are checked with [`ExpressionEvaluationStrategy::Condition`],
...
/// ```
///
/// The two branches of a conditional expression also inherit its evaluation strategy: whichever
...
/// with [`ExpressionEvaluationStrategy::Value`]. The `if` tests the tuple returned by `wrap`, using
/// [`ExpressionEvaluationStrategy::Condition`]. We call these "independent tests": reporting the
    ...
    pub(super) fn check_condition_redundancy(
        ...
        checker.check(
            BooleanTest {
                ...
                truthiness,
                evaluation: ExpressionEvaluationStrategy::Condition,
            },
    ...
    pub(super) fn check_negation_redundancy(
        ...
        checker.check(
            BooleanTest {
                ...
                evaluation: ExpressionEvaluationStrategy::Value,
            },
            RedundantConditionContext::Standalone,
        );
    }
    ...
    pub(super) fn check_suite_for_redundant_if_statements(&self, suite: &[ast::Stmt]) {
        ...
        for (i, statement) in suite.iter().enumerate() {
            ...
            for (branch_index, (test, body)) in branches.enumerate() {
                ...
                checker.check(
                    checker.boolean_test(test, ExpressionEvaluationStrategy::Condition),
                    context,
                );
            }
        }
...
impl<'db> RedundantConditionChecker<'_, 'db> {
    ...
    /// [`ExpressionEvaluationStrategy`]. Return a [`BooleanTest`] instance.
    ///
    ...
    ///
    /// See the [`ExpressionEvaluationStrategy`] doc-comment for more details.
    ...
    fn check_test(
        ...
        match test.expression {
            ...
            ast::Expr::If(ast::ExprIf {
                ...
                self.check(
                    self.boolean_test(test, ExpressionEvaluationStrategy::Condition),
    ...
    fn visit_expr(&mut self, expression: &'ast ast::Expr) {
        ...
        match expression {
            ast::Expr::UnaryOp(ast::ExprUnaryOp {
                ...
                self.checker.check(
                    self.checker
                        .boolean_test(operand, ExpressionEvaluationStrategy::Value),
            ...
            ast::Expr::If(ast::ExprIf {
                ...
                self.checker.check(
                    self.checker
                        .boolean_test(test, ExpressionEvaluationStrategy::Condition),
                    self.condition_context,
                );
