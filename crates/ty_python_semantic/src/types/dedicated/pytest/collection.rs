...
fn has_default_pytest_constructors(db: &dyn Db, class: ClassLiteral<'_>) -> bool {
    ...
    PytestConstructor::ALL.into_iter().all(|constructor| {
        ...
        if actual.provenance == expected.provenance {
            ...
            restored_object_constructor(db, definition) == Some(constructor)
        } else {
            false
        }
    })
...
#[salsa::tracked(returns(copy))]
fn restored_object_constructor<'db>(
    db: &'db dyn Db,
    definition: Definition<'db>,
) -> Option<PytestConstructor> {
    if !matches!(
        definition.kind(db),
    ...
    let value_type = definition_expression_type(db, definition, value);
    let env = ProgramEnvironment::from_definition(definition);
    let object = KnownClass::Object.try_to_class_literal(db, &env)?;

    PytestConstructor::ALL.into_iter().find(|constructor| {
        let Place::Defined(expected) = ClassLiteral::Static(object)
            .class_member(db, &env, constructor.name(), MemberLookupPolicy::default())
            .place
        else {
            return false;
        };
        value_type == expected.ty
    })
}

#[derive(Debug, Clone, Copy, Eq, PartialEq, get_size2::GetSize, salsa::SalsaValue)]
enum PytestConstructor {
    Init,
    New,
}

impl PytestConstructor {
    const ALL: [Self; 2] = [Self::Init, Self::New];
...
default_new = object.__new__
custom_init = InitBase.__init__

class TestRestoredInitAlias(InitBase):
    __init__ = default_init
    def test_restored_init_alias(self): ...

class TestRestoredNewAlias(NewBase):
    __new__ = default_new
    def test_restored_new_alias(self): ...

class TestCustomInitAlias(InitBase):
    __init__ = custom_init
    def test_custom_init_alias(self): ...
"#,
        );
        ...
        assert_eq!(
            ...
            vec![
                ...
                (
                    "test_restored_init_alias".to_owned(),
                    PytestTestKind::Pytest
                ),
                ("test_restored_new".to_owned(), PytestTestKind::Pytest),
                ("test_restored_new_alias".to_owned(), PytestTestKind::Pytest),
            ]
        );
    }

    #[test]
    fn updates_restored_constructor_checks_across_test_modules() {
        const RESTORED_CONSTRUCTOR: &str = r#"
class CustomConstructor:
    def __init__(self): ...

class RestoredConstructor(CustomConstructor):
    __init__ = object.__init__
"#;
        let mut db = pytest_db_with_files(&[
            ("/src/constructors.py", RESTORED_CONSTRUCTOR),
            (
                "/src/test_first.py",
                r#"
from constructors import RestoredConstructor

class TestFirst(RestoredConstructor):
    def test_first(self): ...
"#,
            ),
            (
...
class TestSecond(RestoredConstructor):
...
"#,
        ...
        assert_eq!(
            collected_test_names(&db, "/src/test_first.py"),
            ["test_first"]
        );
        assert_eq!(
            collected_test_names(&db, "/src/test_second.py"),
            ["test_second"]
        );

        db.write_file(
            "/src/constructors.py",
            format!("{RESTORED_CONSTRUCTOR}\nUNRELATED = 1\n"),
        )
        .expect("provider edit should succeed");
        assert_eq!(
            collected_test_names(&db, "/src/test_first.py"),
            ["test_first"]
        );
        ...
        db.write_file(
            "/src/constructors.py",
            RESTORED_CONSTRUCTOR.replace("object.__init__", "CustomConstructor.__init__"),
        )
        .expect("provider edit should succeed");
        assert!(collected_test_names(&db, "/src/test_first.py").is_empty());
        assert!(collected_test_names(&db, "/src/test_second.py").is_empty());
    }
...
"#,
    ...
    fn collected_tests(db: &TestDb, path: &str) -> Vec<(String, PytestTestKind)> {
        let mut tests = pytest_tests_in_file(db, program_file(db, path))
            .iter()
            .filter_map(|test| {
    ...
    fn collected_test_names(db: &TestDb, path: &str) -> Vec<String> {
        collected_tests(db, path)
            .into_iter()
            .map(|(name, _)| name)
            .collect()
    }
