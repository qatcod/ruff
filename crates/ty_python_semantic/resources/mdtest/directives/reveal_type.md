...
An installation of `typing_extensions` without `reveal_type` cannot provide the runtime import,
even if it is declared as a direct dependency and typeshed's bundled stub includes the function. As
such, no import fix is offered.
