...
impl<'resolver, 'db> SubmoduleEnumeration<'resolver, 'db> {
    ...
    fn package_child_names(&self, prefix: &PrefixSearch) -> BTreeSet<String> {
        let context = &self.search.resolver.context;
...
impl<'resolver, 'db> ModuleSearch<'resolver, 'db> {
    ...
    fn overlay_candidates(&self) -> &[ModuleResolutionCandidate] {
        match &self.cursor {
...
enum SearchCandidates {
    Typing(TypingCandidates),
    Runtime(ResolvedNames),
}
...
impl SearchCandidates {
    ...
    fn from_roots(context: &ResolverContext, component: &str) -> Self {
    ...
    fn in_search_path(context: &ResolverContext, prefix: &ModuleName, path: &SearchPath) -> Self {
    ...
    fn enter_package(&self, context: &ResolverContext, component: &str) -> Self {
        match self {
    ...
    fn is_empty(&self, context: &ResolverContext, prefix: &ModuleName) -> bool {
        match self {
            Self::Typing(typing) => {
...
struct TypingCandidates {
    ...
    extra_path_roots: Option<Rc<ResolvedNames>>,
    ...
    full_search_candidates: OnceCell<ResolvedNames>,
}

impl TypingCandidates {
    fn from_roots(context: &ResolverContext, component: &str) -> Self {
        let (stubs, _) =
